...
int main(int argc, char *argv[])
{
    ...
    int sizes[commsize];
    int segment_size = (JSIZE - 2) / commsize;
    int start_elem = rank * segment_size;
    int end_elem = (rank == commsize - 1) ? JSIZE - 2 : start_elem + segment_size;

    int size = end_elem - start_elem;

    if (rank == 0)
        for (int i = 0; i < commsize; i++)
            sizes[i] = (i == commsize - 1) ? JSIZE - 2 - i * segment_size : segment_size;
    ...
    for (int i = 3; i < ISIZE; ++i)
    {
        ...
        if (rank != 0)
        {
            MPI_Send(&(a[i][start_elem]), 2, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD);
        }
        if (rank != commsize - 1)
        {
            MPI_Recv(&(a[i][end_elem]), 2, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
    }
